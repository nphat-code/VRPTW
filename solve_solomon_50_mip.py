...
def read_solomon(file_path, n_customers=50):
    ...
    # Lấy sức tải xe (Capacity)
    capacity = int(lines[4].strip().split()[1])
    
    data = []
    # Đọc Depot + n_customers (tổng cộng n_customers + 1 dòng dữ liệu)
...
def solve_vrptw_50(data, capacity):
    ...
    # Cài đặt giới hạn thời gian (50 khách hàng có thể cần nhiều thời gian hơn)
    model.max_gap = 0.05 # Chấp nhận sai số 5%
    status = model.optimize(max_seconds=120) # Tăng lên 120 giây
...
if __name__ == "__main__":
    ...
    if data:
        ...
        if routes:
            ...
            save_path = f'route_images/solution_50_mip_{base_name}.png'
